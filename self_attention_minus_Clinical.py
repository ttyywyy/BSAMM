...
class Surv_attention(nn.Module):
    ...
    def __init__(self):
        ...
        self.for_head = nn.Sequential(
            nn.Linear(768, 32,bias=False), nn.ReLU(), nn.Dropout(0.5),
    ...
    def forward(self,  mri, wsi,ich):
        MRI_feat = self.MRI(mri)
        combined_tensor = torch.stack([MRI_feat, wsi, ich], dim=1)
...
class MultimodalDataset(Dataset):
    ...
    augment=True 时：
      ...
      3) 再切回三段
    """
    def __init__(self, mri, wsi, ich, surv,
        ...
        self.len_m, self.len_w, self.len_ii = self.mri.shape[1], self.wsi.shape[1], self.ich.shape[1]
    ...
    def __getitem__(self, i):
        m, w, ii = self.mri[i], self.wsi[i], self.ich[i]
        if self.aug and self.jr != 0:
            vec = torch.cat([m, w, ii])
        ...
        return { "mri": m, "wsi": w, "ich": ii, "time": time, "event": event}
...
def run_fold(tr_idx, va_idx,
    ...
    ds_tr = MultimodalDataset(mri.loc[tr_idx], wsi.loc[tr_idx],ich.loc[tr_idx],
                              surv.loc[tr_idx], augment=True)
    ds_va = MultimodalDataset(mri.loc[va_idx], wsi.loc[va_idx],ich.loc[va_idx],
                              surv.loc[va_idx], augment=False)

    dl_tr = DataLoader(ds_tr, bs, shuffle=True,  collate_fn=collate)
    dl_va = DataLoader(ds_va, 128 , shuffle=False, collate_fn=collate)
...
# ───────── 主函数 ─────────
def main():
    data_dir = "/Volumes/T7/WSI/features/melting/train"
    out_dir = "/Volumes/T7/WSI/features/melting/train/pth";os.makedirs(out_dir, exist_ok=True)
    bs,epochs,lr,wd = 128,500,3e-6,1
