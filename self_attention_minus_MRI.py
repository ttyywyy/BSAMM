...
class Surv_attention(nn.Module):
    ...
    def forward(self, wsi,ich,text):
        #MRI_feat = self.MRI(mri)
        combined_tensor = torch.stack([wsi, ich, text], dim=1)
...
class MultimodalDataset(Dataset):
    ...
    augment=True 时：
      ...
      3) 再切回三段
    """
    def __init__(self, wsi, ich, txt, surv,
        ...
        self.len_w, self.len_ii, self.len_t = self.wsi.shape[1], self.ich.shape[1], self.txt.shape[1]
    ...
    def __getitem__(self, i):
        w, ii, t = self.wsi[i], self.ich[i], self.txt[i]
        if self.aug and self.jr != 0:
            vec = torch.cat([w, ii, t])
        ...
        return {"wsi": w, "ich": ii, "text": t, "time": time, "event": event}
...
def run_fold(tr_idx, va_idx,
    ...
    ds_tr = MultimodalDataset(wsi.loc[tr_idx],ich.loc[tr_idx],txt.loc[tr_idx],
                              surv.loc[tr_idx], augment=True)
    ds_va = MultimodalDataset(wsi.loc[va_idx],ich.loc[va_idx],txt.loc[va_idx],
    ...
    for ep in range(1, epochs+1):
        ...
        with torch.no_grad():
            for b in dl_va:
                for k,v in b.items():
                    if torch.is_tensor(v): b[k]=v.to(dev)
                r_val = model(b['wsi'], b['ich'],b['text'])
                r_va.append(r_val)
...
def main():
    ...
    print(f"样本量: {len(surv_val)} ▶ 开始 5-fold CV")
