...
model_path    = "/Volumes/T7/WSI/features/melting/train/pth/best_overall_wsi.pth"
path_risk     = "/Volumes/T7/WSI/features/melting/train/risk_out_train-WSI.xlsx"
# ==========================================================
...
class MultimodalDataset(Dataset):
    def __init__(self, mri, ich, text, surv):
    ...
    def __getitem__(self, idx):
        return (self.mri[idx], self.ich[idx],
...
def main():
    ...
    ds_test = MultimodalDataset(mri, ich, text, surv)
    ...
    with torch.no_grad():
        for mri_b, ich_b, txt_b, t_b, e_b in dl_test:
            mri_b = mri_b.to(device); ich_b = ich_b.to(device); txt_b = txt_b.to(device)
            r= model(mri_b, ich_b, txt_b)
