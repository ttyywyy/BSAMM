...
model_path    = "/Volumes/T7/WSI/features/melting/train/pth/best_overall_IHC.pth"
path_risk     = "/Volumes/T7/WSI/features/melting/train/risk_out_train-IHC.xlsx"
# ==========================================================
...
class MultimodalDataset(Dataset):
    def __init__(self, mri, wsi, text, surv):
    ...
    def __getitem__(self, idx):
        return (self.mri[idx], self.wsi[idx],
...
def main():
    ...
    surv = pd.read_excel(os.path.join(data_dir_test, "rs63_train.xlsx"), index_col=0)
    print(f"▶ 外部测试集样本数: {len(surv)}")



    ds_test = MultimodalDataset(mri, wsi, text, surv)
    ...
    with torch.no_grad():
        for mri_b,  wsi_b, txt_b, t_b, e_b in dl_test:
            mri_b = mri_b.to(device); wsi_b = wsi_b.to(device); txt_b = txt_b.to(device)
            r= model(mri_b, wsi_b,txt_b)
