...
class Surv_attention(nn.Module):
    ...
    def forward(self,mri,ich,text):
        MRI_feat = self.MRI(mri)
        combined_tensor = torch.stack([MRI_feat,ich, text], dim=1)
...
class MultimodalDataset(Dataset):
    ...
    augment=True 时：
      ...
      3) 再切回三段
    """
    def __init__(self, mri, ich, txt, surv,
        ...
        self.len_m, self.len_ii, self.len_t = self.mri.shape[1], self.ich.shape[1], self.txt.shape[1]
...
def run_fold(tr_idx, va_idx,
    ...
    ds_tr = MultimodalDataset(mri.loc[tr_idx], ich.loc[tr_idx],txt.loc[tr_idx],
                              surv.loc[tr_idx], augment=True)
    ds_va = MultimodalDataset(mri.loc[va_idx],ich.loc[va_idx],txt.loc[va_idx],
    ...
    for ep in range(1, epochs+1):
        ...
        with torch.no_grad():
            for b in dl_va:
                for k,v in b.items():
                    if torch.is_tensor(v): b[k]=v.to(dev)
                r_val = model( b['mri'],b['ich'],b['text'])
                r_va.append(r_val)
