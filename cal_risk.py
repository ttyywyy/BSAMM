...
model_path    = "/Volumes/T7/WSI/features/train_new5/pth/best_overall.pth"
...
class DeepSurvMLP(torch.nn.Module):
    def __init__(self):
        ...
        self.mlp = torch.nn.Sequential(
            torch.nn.Linear(2304, 128), torch.nn.ReLU(), torch.nn.Dropout(0.5),
    ...
    def forward(self, wsi, mri, text):
        z1 = self.patch_SR(wsi); z2 = self.MRI(mri)
        x  = torch.cat([z1, z2, text], dim=1)
        return self.mlp(x).squeeze(-1)

# ----------------- Dataset（无抖动） -------------------
class MultimodalDataset(Dataset):
    def __init__(self, wsi, mri, text, surv):
    ...
    def __getitem__(self, idx):
        return (self.wsi[idx], self.mri[idx], self.text[idx],
...
def main():
    ...
    test_ds = MultimodalDataset(wsi, mri, text, surv)
    ...
    risks, times, events = [], [], []
    with torch.no_grad():
        for wsi_b, mri_b, txt_b, t_b, e_b in test_loader:
            wsi_b = wsi_b.to(device); mri_b = mri_b.to(device); txt_b = txt_b.to(device)
            r = model(wsi_b, mri_b, txt_b).cpu()
