...
import sys
# ===== 路径自行替换 =====
data_dir_test = "/Volumes/T7/WSI/features/melting/train"
model_path    = "/Volumes/T7/WSI/features/melting/train/pth/best_overall_mri.pth"
path_risk     = "/Volumes/T7/WSI/features/melting/train/risk_out_train-MRI.xlsx"
# ==========================================================
...
class MultimodalDataset(Dataset):
    def __init__(self, wsi, ich, text, surv):
    ...
    def __getitem__(self, idx):
        return (self.wsi[idx], self.ich[idx],
...
def main():
    ...
    out_dir = '/Volumes/T7/WSI/features/train_new6'
    #log_path = os.path.join(out_dir, "pretrained_val_log.txt")
    ...
    ds_test = MultimodalDataset(wsi, ich, text, surv)
    ...
    with torch.no_grad():
        for  wsi_b, ich_b, txt_b, t_b, e_b in dl_test:
            wsi_b = wsi_b.to(device); ich_b = ich_b.to(device); txt_b = txt_b.to(device)
            r= model(wsi_b, ich_b, txt_b)
